...
class PolicyNet(torch.nn.Module):
    ...
    def forward(self, x):
        x = F.relu(self.fc1(x))
        return (torch.tanh(self.fc2(x)) + 1) * self.action_bound / 2
...
class DDPG:
    ...
    def update(self, transition_dict):
        ...
        truncated = torch.tensor(np.array(transition_dict['truncated']), dtype=torch.float).view(-1, 1).to(self.device)

        next_q_values = self.target_critic(next_states, self.target_actor(next_states))
        q_targets = rewards + self.gamma * next_q_values * (1 - dones) * (1 - truncated)
